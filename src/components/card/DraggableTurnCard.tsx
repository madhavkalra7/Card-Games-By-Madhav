...
export const DraggableTurnCard: React.FC<DraggableTurnCardProps> = ({
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center pointer-events-auto select-none">
      
      {/* 30s Turn Timer Pill */}
      <div className="mb-2 flex items-center gap-2 bg-black/80 backdrop-blur-md px-3.5 py-1 rounded-full border border-gold/50 shadow-xl">
        <span className="text-[11px] text-zinc-300 font-semibold">Your Turn:</span>
        <span
          className={cn(
      ...
      <div className="relative flex flex-col items-center">
        ...
        >
          <div className="relative shadow-2xl rounded-[12px] ring-4 ring-gold/90 animate-pulse-gold">
            <PlayingCard card={card} size="lg" glow={true} />
            
            {/* Drag Handle Floating Badge */}
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gold text-black text-[10px] font-black px-2.5 py-0.5 rounded-full shadow-lg border border-yellow-200 flex items-center gap-1 whitespace-nowrap">
        ...
        <div className="mt-2 text-center">
          <p className="text-[11px] text-amber-200/90 font-bold tracking-wide drop-shadow">
            Drag & drop card on <span className="text-gold">Center</span> or any <span className="text-gold">Right Deck</span>
          </p>
        </div>
      </div>
