        ...
        <div className="flex-1 overflow-y-auto p-3.5 sm:p-5 space-y-4">
          ...
          <form onSubmit={handleQuickAdd} className="pt-2 border-t border-white/10">
            <label className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 mb-1.5 block">
              Quick Add & Invite by Nickname
            </label>
            <div className="flex items-center gap-2">
              <input
                type="text"
                placeholder="Enter player nickname or email..."
                value={quickAddInput}
                onChange={(e) => setQuickAddInput(e.target.value)}
                className="flex-1 px-3 py-2 rounded-xl bg-zinc-900 border border-zinc-700 text-white placeholder-zinc-500 text-xs focus:outline-none focus:border-gold"
              />
              <button
                type="submit"
                disabled={isAdding || !quickAddInput.trim()}
                className={cn(
                  'px-3 py-2 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-1 transition-all cursor-pointer',
