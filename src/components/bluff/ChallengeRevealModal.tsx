  ...
  return (
    ...
    >
      <div
        ...
        className={cn(
          'relative w-full max-w-lg rounded-3xl border-2 p-5 sm:p-7 shadow-2xl overflow-hidden text-center transition-all animate-in zoom-in-90 duration-300',
          wasBluff
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-zinc-400 hover:text-white p-1.5 rounded-full hover:bg-white/10 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex items-center justify-center gap-2 mb-3">
          {wasBluff ? (
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/20 border border-red-500/50 text-red-400 text-xs font-black uppercase tracking-widest animate-pulse">
              <ShieldAlert className="w-4 h-4" />
              <span>SHOWDOWN • BLUFF DETECTED</span>
            </div>
          ) : (
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/50 text-emerald-400 text-xs font-black uppercase tracking-widest animate-pulse">
        ...
        <div className="my-5 flex items-center justify-center gap-2 sm:gap-3 flex-wrap">
          {revealedCards.map((card, idx) => {
            const isMatch = card.rank === declaredRank;
            return (
              <div key={idx} className="flex flex-col items-center gap-1.5 animate-in slide-in-from-bottom-3 duration-300">
                <PlayingCard
                  card={card}
                  size="sm"
                  className={cn(
                    ...
                    isMatch
                      ...
                      : 'ring-3 ring-red-500 shadow-red-500/50 scale-105'
                  )}
                />
                <span
                  className={cn(
        ...
        <div className="mb-4">
          <h2
            className={cn(
              'text-2xl sm:text-3xl font-black uppercase tracking-wide font-serif',
              wasBluff ? 'text-red-400' : 'text-emerald-400'
            )}
          >
            {wasBluff ? '🚨 BLUFF CAUGHT!' : '🛡️ 100% HONEST!'}
          </h2>
          <p className="text-xs sm:text-sm text-zinc-200 mt-2 font-medium">
            {wasBluff ? (
              <>
        ...
        <button
          type="button"
          onClick={onClose}
          className="w-full py-2.5 sm:py-3 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-black font-black text-xs sm:text-sm uppercase tracking-wider shadow-gold-glow transition-all active:scale-95 cursor-pointer"
        >
          Continue Game
        </button>
      </div>
